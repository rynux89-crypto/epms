...
UPDATE hm
SET
    hm.max_voltage = s.max_voltage,
    hm.min_voltage = s.min_voltage
FROM dbo.hourly_measurements hm
INNER JOIN hourly_src s
    ON s.meter_id = hm.meter_id
   AND s.measured_hour = hm.measured_hour;
...
UPDATE dm
SET
    dm.max_voltage = s.max_voltage,
    dm.min_voltage = s.min_voltage
FROM dbo.daily_measurements dm
INNER JOIN daily_src s
    ON s.meter_id = dm.meter_id
   AND s.measured_date = dm.measured_date;
...
UPDATE mm
SET
    mm.max_voltage = s.max_voltage,
    mm.min_voltage = s.min_voltage
FROM dbo.monthly_measurements mm
INNER JOIN monthly_src s
    ON s.meter_id = mm.meter_id
   AND s.measured_month = mm.measured_month;
...
UPDATE ym
SET
    ym.max_voltage = s.max_voltage,
    ym.min_voltage = s.min_voltage
FROM dbo.yearly_measurements ym
INNER JOIN yearly_src s
    ON s.meter_id = ym.meter_id
   AND s.measured_year = s.measured_year;

...
<%! 
    ...
    private static float maxAbs(List<Float> a) {
        if (a == null || a.isEmpty()) return 0f;
        float m = 0f;
        for (float v : a) {
            float av = Math.abs(v);
...
<%
    ...
    // 기준 선택: avg(조회구간 평균) / rated(정격 기준)
    String refMode = request.getParameter("refMode");
    if (refMode == null || refMode.trim().isEmpty()) refMode = "avg";

    float ratedVDefault = 380f; // 필요한 현장 정격(예: 380V)으로 조정
    float ratedV = safeParseFloat(request.getParameter("ratedV"), ratedVDefault);
...
<form method="GET">
    <div class="ref-box">
        ...
        <label><input type="radio" name="refMode" value="rated" <%= "rated".equals(refMode) ? "checked" : "" %> > 정격</label>
        <label>정격전압(V): <input type="number" name="ratedV" step="0.1" value="<%= ratedV %>"></label>
    </div>
...
<%
    ...
    List<Float> voltageRateAB = new ArrayList<>();
    List<Float> voltageRateBC = new ArrayList<>();
    List<Float> voltageRateCA = new ArrayList<>();
    List<Float> voltageUpper = new ArrayList<>();
    List<Float> voltageLower = new ArrayList<>();

    List<Float> vabRaw = new ArrayList<>();
    List<Float> vbcRaw = new ArrayList<>();
    List<Float> vcaRaw = new ArrayList<>();

    float vabAvg = 0f, vbcAvg = 0f, vcaAvg = 0f;
    float vabRef = 0f, vbcRef = 0f, vcaRef = 0f;

    try {
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT measured_at, voltage_ab, voltage_bc, voltage_ca ")
    ...
    if (!vabRaw.isEmpty()) {
        float vabSum = 0f, vbcSum = 0f, vcaSum = 0f;
        for (int i = 0; i < vabRaw.size(); i++) {
            vabSum += vabRaw.get(i);
            vbcSum += vbcRaw.get(i);
            vcaSum += vcaRaw.get(i);
        }
        vabAvg = vabSum / vabRaw.size();
        vbcAvg = vbcSum / vbcRaw.size();
        vcaAvg = vcaSum / vcaRaw.size();

        // 기준값 결정
        if ("rated".equals(refMode) && Math.abs(ratedV) > EPS) {
            vabRef = ratedV;
            vbcRef = ratedV;
            vcaRef = ratedV;
        } else {
        ...
        boolean vabSafe = Math.abs(vabRef) > EPS;
        boolean vbcSafe = Math.abs(vbcRef) > EPS;
        boolean vcaSafe = Math.abs(vcaRef) > EPS;
        ...
        for (int i = 0; i < labels.size(); i++) {
            ...
            voltageRateAB.add(vabSafe ? ((vab - vabRef) / vabRef * 100f) : 0f);
            voltageRateBC.add(vbcSafe ? ((vbc - vbcRef) / vbcRef * 100f) : 0f);
            voltageRateCA.add(vcaSafe ? ((vca - vcaRef) / vcaRef * 100f) : 0f);
    ...
    float abMax = maxFloat(voltageRateAB), abMin = minFloat(voltageRateAB), abP95 = percentileAbs(voltageRateAB, 95);
    float bcMax = maxFloat(voltageRateBC), bcMin = minFloat(voltageRateBC), bcP95 = percentileAbs(voltageRateBC, 95);
    float caMax = maxFloat(voltageRateCA), caMin = minFloat(voltageRateCA), caP95 = percentileAbs(voltageRateCA, 95);
...
<div class="summary-wrap">
    ...
    <div class="summary-meta">
        <span class="badge">기준: <%= "rated".equals(refMode) ? ("정격 " + ratedV + "V") : "조회구간 평균" %></span>
        <span class="badge <%= ("Vab".equals(worst) ? "badge-worst" : "") %>">Worst: <%= worst %> (P95 |변동| = <%= String.format(java.util.Locale.US, "%.2f", worstScore) %>%)</span>
        <span class="badge badge-line">Va 기준값: <%= String.format(java.util.Locale.US, "%.2f", vabRef) %> V</span>
        <span class="badge badge-line">Vb 기준값: <%= String.format(java.util.Locale.US, "%.2f", vbcRef) %> V</span>
        <span class="badge badge-line">Vc 기준값: <%= String.format(java.util.Locale.US, "%.2f", vcaRef) %> V</span>
    </div>
    ...
    <table class="summary-table">
        ...
        <tbody>
        <tr class="<%= "Vab".equals(worst) ? "worst-row" : "" %>">
            <td>Vab</td>
            <td><%= String.format(java.util.Locale.US, "%.2f", abMin) %></td>
            <td><%= String.format(java.util.Locale.US, "%.2f", abMax) %></td>
            <td><%= String.format(java.util.Locale.US, "%.2f", abP95) %></td>
            <td><%= "Vab".equals(worst) ? "Worst" : "" %></td>
        </tr>
        <tr class="<%= "Vbc".equals(worst) ? "worst-row" : "" %>">
            <td>Vbc</td>
            <td><%= String.format(java.util.Locale.US, "%.2f", bcMin) %></td>
            <td><%= String.format(java.util.Locale.US, "%.2f", bcMax) %></td>
            <td><%= String.format(java.util.Locale.US, "%.2f", bcP95) %></td>
            <td><%= "Vbc".equals(worst) ? "Worst" : "" %></td>
        </tr>
        <tr class="<%= "Vca".equals(worst) ? "worst-row" : "" %>">
            <td>Vca</td>
            <td><%= String.format(java.util.Locale.US, "%.2f", caMin) %></td>
            <td><%= String.format(java.util.Locale.US, "%.2f", caMax) %></td>
            <td><%= String.format(java.util.Locale.US, "%.2f", caP95) %></td>
            <td><%= "Vca".equals(worst) ? "Worst" : "" %></td>
        </tr>
        </tbody>
    </table>
</div>
...
<div class="chart-container">
    <div id="variationVoltageChart" style="width:100%; height:100%;"></div>
</div>
</div>
...
const voltageRateAB = [<%= voltageRateAB.stream().map(String::valueOf).collect(java.util.stream.Collectors.joining(",")) %>];
const voltageRateBC = [<%= voltageRateBC.stream().map(String::valueOf).collect(java.util.stream.Collectors.joining(",")) %>];
const voltageRateCA = [<%= voltageRateCA.stream().map(String::valueOf).collect(java.util.stream.Collectors.joining(",")) %>];
const voltageUpper  = [<%= voltageUpper.stream().map(String::valueOf).collect(java.util.stream.Collectors.joining(",")) %>];
const voltageLower  = [<%= voltageLower.stream().map(String::valueOf).collect(java.util.stream.Collectors.joining(",")) %>];
...
const voltageRateABPlot = pickByIndex(voltageRateAB, sampleIdx);
const voltageRateBCPlot = pickByIndex(voltageRateBC, sampleIdx);
const voltageRateCAPlot = pickByIndex(voltageRateCA, sampleIdx);
const voltageUpperPlot = pickByIndex(voltageUpper, sampleIdx);
const voltageLowerPlot = pickByIndex(voltageLower, sampleIdx);

const worst = "<%= worst %>";

const chartDom = document.getElementById('variationVoltageChart');
...
const option = {
  ...
  series: [
    createLargeAreaSeries('Vab 변동율', voltageRateABPlot, 'Vab', '#ff6b6b'),
    createLargeAreaSeries('Vbc 변동율', voltageRateBCPlot, 'Vbc', '#4dabf7'),
    createLargeAreaSeries('Vca 변동율', voltageRateCAPlot, 'Vca', '#51cf66'),
    { name: '전압 상한(+5%)', type: 'line', data: voltageUpperPlot, showSymbol: false, lineStyle: { type: 'dashed', width: 1 } },
    { name: '전압 하한(-5%)', type: 'line', data: voltageLowerPlot, showSymbol: false, lineStyle: { type: 'dashed', width: 1 } }
  ]
};
